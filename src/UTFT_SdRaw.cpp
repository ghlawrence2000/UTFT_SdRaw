...
int UTFT_SdRaw::loadS(int x, int y, char *filename, int bufmult, bool iswap)
{
  Serial.println("HELLO"); // Tell host we are ready
  Serial.println(filename);
  while (!Serial.find("IMAGE")); // Wait for IMAGE command from host
  uint32_t sx, sy;
  byte buf1[2];
  Serial.readBytes(buf1, 2);
  sx = (((uint32_t)buf1[1] << 8) |
                   (uint32_t)buf1[0]) ;
  Serial.readBytes(buf1, 2);
  sy = (((uint32_t)buf1[1] << 8) |
                   (uint32_t)buf1[0]) ;

  byte buf[2 * bufmult * sx];
  int cx, cy, cp;
  word result;
  while((cy=Serial.available())==0);
  if (cy=(Serial.available()) > 0)
  {
    cbi(_UTFT->P_CS, _UTFT->B_CS);
    cx = 0;
    cy = 0;
